...
/* 插入排序 Time Complexity: 0（n²）
 * 說明：最壞情況下需要進行 n-1 輪，每一輪最多需要比校 n-i-1 次（i 為當前輪數），
 * 因此總比校次數為 （n-1）+（n-2）+ ... + 1 = n(n-1/2，時間複雜度為 0(n²）。
 */

/*
 * 二分插入排序 Time Complexity: （n log n）
 * 說明：使用二分搜尋找到插入位置，減少了比校次數，但仍需移動元素，整體時間複雜度為 0(n log n）。
 /*

/*
 * 說明：在已排序的區間中使用二分搜尋，找到插入位置後再進行元素移動，減少了比校次數。
 */
